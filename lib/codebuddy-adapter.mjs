...
export function serializeMessages(messages, supportsImages, images) {
  ...
  for (const message of messages) {
    ...
    for (const r of results) {
      // The wire's `tool` entry cannot carry image parts, so a tool-result
      // image contributes its text only.
...
export class CodebuddyAdapter extends LlmAdapter {
  ...
  async *stream(options) {
    ...
    const payload = JSON.stringify(serializeRequest(options, supportsImages, await this.prepareImages(options, options.signal)));
    let response;
    try {
